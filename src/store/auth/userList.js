...
export const getUserInfo = createAsyncThunk(
  ...
  export const singleUserInfo = createAsyncThunk(
    ...
    async (data) => {
      try {
        const response = await axios.get(
            `https://sozhiyavellalarmarriage.com/matrimonyApp/UserController/getUserDetails`,
            data,
            {
              headers: {
  ...
  export const userListSlice = createSlice({
    ...
    reducers: {
      clearUserInfo: (state) => {
        ...
        state.token = null;
        state.singleUserInfo = undefined;
      },
    },
    extraReducers(build) {
      build
        ...
        .addCase(singleUserInfo.pending, (state) => {
          state.singleUserInfo = {
            loading: true,
          };
        })
        .addCase(singleUserInfo.fulfilled, (state, action) => {
          state.singleUserInfo = {
            isGet: action.payload,
            loading: false,
          };
        })
        .addCase(singleUserInfo.rejected, (state, action) => {
          state.singleUserInfo = {
            isError: action?.error?.message,
            loading: false,
          };
